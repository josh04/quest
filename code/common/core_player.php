...
class core_player implements basic_player {
    ...
    public $disabled = 0;
    public $hooks; // sneaky sneaky sneak sneak
    public $only_update_hooks = false;
    public $joins = array();
    ...
    * @return core_player this object
    */
    public function load_core($common) {
        ...
        if (isset($common->hooks)) {
            $this->hooks = clone $common->hooks; // we're playing with FIRE here!

            $this->hooks->common =& $this;
        }

        foreach ($common->player_flags as $flag) {
            $var_name = "do_".$flag;
    ...
    */
    protected function halt_if_suspended() {
        if ($this->disabled == "1" && $this->page !="ticket") {
            return true;
        }
        return false;
    }
    ...
    */
    public function get_player($identity) {
        ...
        $this->hooks->get("player/get/post");
        return $return;
    }
