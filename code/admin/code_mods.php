...
class code_mods extends _code_admin {
    ...
    */
    public function mods_switch() {
        if ($_GET['act']=="edit" && isset($_GET['id'])) {
            $mods_switch = $this->single_hook($_GET['id']);
        } elseif($_GET['act']=="install" && isset($_GET['id'])) {
            if(isset($_GET['confirmed'])) $mods_switch = $this->install_hook($_GET['id']);
            else $mods_switch = $this->install_hook_preface($_GET['id']);
        } else {
    ...
    public function mods_home($message="") {
        ...
        if ($dh = opendir($dir)) {
            while (($file = readdir($dh)) !== false) {
                if(filetype($dir.'/'.$file)=="dir" && !in_array($file,array('.','..'))) $hooks[] = $file;
            }
            closedir($dh);
        }

        if(isset($_GET['id'])) $hooks = array($_GET['id']);

        foreach($hooks as $hook) {
            $hooks_rows .= $this->parse_hook($hook);
        }

        if(empty($hooks_rows)) $message = $this->skin->error_box($this->lang->mod_not_found);
    ...
    */
    public function parse_hook($hook) {
        ...
        if(isset($xml->authorurl)) $xml->author = "<a href='".$xml->authorurl."'>".$xml->author."</a>";

        if($installed) {
            if(isset($xml->admin)) $action = $this->skin->edit_link($hook);
        } else {
    ...
    */
    public function single_hook($hook) {
        ...
        $path = "././hooks/".$hook."/config.xml";
        if(!file_exists($path)) return $this->mods_home($this->skin->error_box($this->lang->mod_no_configure));
        $xml = simplexml_load_file($path);

        if(!isset($xml->admin)) return $this->mods_home($this->skin->error_box($this->lang->mod_no_configure));
    ...
    */
    public function install_hook_preface($hook) {
        ...
        if($xml->hook) $install_guide .= "<li>Hook into <em>".$xml->hook->attributes()."</em></li>";

        if($xml->settings) foreach($xml->settings->children() as $key=>$value) {
            if($value) $key = $key." (default: ".$value.")";
            $install_guide .= "<li>Register the setting <em>".$key."</em></li>";
        }

        if($xml->sql) $install_guide .= "<li>Execute the following SQL:<br /><div class='quest-select'>".$xml->sql."</div></li>";
    ...
    */
    public function install_hook($hook) {
        ...
        if($xml->hook) {
            ...
            if(isset($xml->admin->file)) array_push($vals, strval($xml->admin->file));
        ...
        if($xml->sql) {$this->db->execute($xml->sql);$returns .= "<br />SQL executed";}

        if($xml->admin->file) $or_admin = $this->skin->or_admin_link($hook);
